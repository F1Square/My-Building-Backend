...
async function adjustBalance(building_id, delta) {
  // delta = positive for inflow, negative for outflow
  const { data: fund } = await supabase
    .from('society_funds').select('current_balance').eq('building_id', building_id).single();

  const current = parseFloat(fund?.current_balance || 0);
  const newBalance = current + delta;

  await supabase.from('society_funds')
    .upsert({ building_id, current_balance: newBalance, updated_at: new Date().toISOString() }, { onConflict: 'building_id' });
...
exports.getFundSummary = async (req, res) => {
  ...
  res.json(data || { building_id, opening_balance: null, current_balance: 0 });
};

// ── Set opening balance (pramukh first-time setup or admin) ──────────────────
exports.setOpeningBalance = async (req, res) => {
  const { amount, building_id: bodyBuildingId } = req.body;
  ...
  let error;
  if (existing) {
    ({ error } = await supabase.from('society_funds').update(payload).eq('building_id', building_id));
  } else {
  ...
  // After setting opening balance, recalc by summing all existing entries on top
  const { data: existingEntries } = await supabase
    .from('expense_entries').select('type, amount').eq('building_id', building_id);
  let balance = parsed;
...
exports.addEntry = async (req, res) => {
  const { type, amount, description, category, date, building_id: bodyBuildingId } = req.body;
  const building_id = req.user.building_id || bodyBuildingId;
  if (!building_id) return res.status(400).json({ error: 'building_id required' });
  ...
  const { data, error } = await supabase
    .from('expense_entries')
    .insert({
      building_id, type, amount: parsed,
  ...
  const balance = await adjustBalance(building_id, delta);
...
exports.editEntry = async (req, res) => {
  ...
  const balance = await adjustBalance(original.building_id, oldDelta + newDelta);
...
exports.deleteEntry = async (req, res) => {
  ...
  const { data: entry } = await supabase
    .from('expense_entries').select('building_id, type, amount').eq('id', id).single();
  ...
  const balance = await adjustBalance(entry.building_id, delta);
