...
// Resident signals intent to pay in cash; pramukh/admin gets notified and
// later flips the record to 'paid' via /approve once the money is collected.
router.post('/payments/:id/request-cash', authenticate, requireRole('user', 'pramukh'), c.requestCashPayment);
...
router.post('/pay/easebuzz-callback', c.easebuzzCallback);         // Easebuzz Redirect
router.get('/pay/easebuzz-callback', c.easebuzzCallback);          // GET fallback
router.post('/pay/phonepe-callback', c.easebuzzCallback);          // Backward compatibility
router.get('/pay/phonepe-callback', c.easebuzzCallback);           // Backward compatibility
...
router.post('/advance/easebuzz-callback', c2.easebuzzCallback);
router.get('/advance/easebuzz-callback', c2.easebuzzCallback);
router.post('/advance/phonepe-callback', c2.easebuzzCallback); // Backward compatibility
router.get('/advance/phonepe-callback', c2.easebuzzCallback);  // Backward compatibility
